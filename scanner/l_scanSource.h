# ifndef xyl_scanSource
# define xyl_scanSource
...
# define l_scan_BeginSource l_scan_BeginSourceFile

extern void l_scan_SetEncoding ARGS ((int yyEncoding, int yyEndian));

   /*
      Specify the encoding and the endian property of the input stream.
      The arguments have to be values as defined above.
      This function has to be called after the function l_scan_BeginSource... .
      ...
      The function l_scan_GetWLine will convert the input stream to a stream
      of type wchar_t.
   */

extern int  l_scan_BeginSourceFile ARGS ((char * yyFileName));

   /*
      l_scan_BeginSourceFile is called from the scanner function l_scan_BeginFile
      ...
      call 'open' or any other handle understood by the function l_scan_GetLine.
   */

# if HAVE_WCHAR_T
extern int  l_scan_BeginSourceFileW ARGS ((wchar_t * yyFileName));

   /*
      Same as l_scan_BeginSourceFile for type wchar_t instead of type char.
      The Source module has to be extended by the user in order to
      implement this feature.
   */
# endif

extern void l_scan_BeginSourceMemory ARGS ((void * yyInputPtr));

   /*
      l_scan_BeginSourceMemory is called from the scanner function l_scan_BeginMemory
...
extern void l_scan_BeginSourceMemoryN ARGS ((void * yyInputPtr, int yyLength));

   /*
      l_scan_BeginSourceMemoryN is called from the scanner function l_scan_BeginMemoryN
...
extern void l_scan_BeginSourceGeneric ARGS ((void * yyInputPtr));

   /*
      l_scan_BeginSourceGeneric is called from the scanner function l_scan_BeginGeneric
      indicating that the input is user-defined at location 'InputPtr'.
      The Source module has to be extended by the user in order to
      implement this feature.
   */

extern int  l_scan_GetLine ARGS ((int yyFile, char * yyBuffer, int yySize));

   /*
      l_scan_GetLine is called from the scanner in order to fill a buffer at
...
# if HAVE_WCHAR_T
extern int  l_scan_GetWLine ARGS ((int yyFile, wchar_t * yyBuffer, int yySize));

   /*
      Same as l_scan_GetLine for type wchar_t instead of type char.
   */
# endif

extern void l_scan_CloseSource ARGS ((int yyFile));

   /*
      l_scan_CloseSource is called from the scanner function l_scan_CloseFile
      at end of file or at end of input, respectively.
      It can be used to close files.
      The functions l_scan_BeginSource... and l_scan_CloseSource can be called
      ...
      Therefore after a call of l_scan_CloseSource the properties of the
