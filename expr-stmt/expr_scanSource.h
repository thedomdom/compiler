# ifndef xyexpr_scanSource
# define xyexpr_scanSource
...
# define expr_scan_BeginSource expr_scan_BeginSourceFile

extern void expr_scan_SetEncoding ARGS ((int yyEncoding, int yyEndian));

   /*
      Specify the encoding and the endian property of the input stream.
      The arguments have to be values as defined above.
      This function has to be called after the function expr_scan_BeginSource... .
      ...
      The function expr_scan_GetWLine will convert the input stream to a stream
      of type wchar_t.
   */

extern int  expr_scan_BeginSourceFile ARGS ((char * yyFileName));

   /*
      expr_scan_BeginSourceFile is called from the scanner function expr_scan_BeginFile
      ...
      call 'open' or any other handle understood by the function expr_scan_GetLine.
   */

# if HAVE_WCHAR_T
extern int  expr_scan_BeginSourceFileW ARGS ((wchar_t * yyFileName));

   /*
      Same as expr_scan_BeginSourceFile for type wchar_t instead of type char.
      The Source module has to be extended by the user in order to
      implement this feature.
   */
# endif

extern void expr_scan_BeginSourceMemory ARGS ((void * yyInputPtr));

   /*
      expr_scan_BeginSourceMemory is called from the scanner function expr_scan_BeginMemory
...
extern void expr_scan_BeginSourceMemoryN ARGS ((void * yyInputPtr, int yyLength));

   /*
      expr_scan_BeginSourceMemoryN is called from the scanner function expr_scan_BeginMemoryN
...
extern void expr_scan_BeginSourceGeneric ARGS ((void * yyInputPtr));

   /*
      expr_scan_BeginSourceGeneric is called from the scanner function expr_scan_BeginGeneric
      indicating that the input is user-defined at location 'InputPtr'.
      The Source module has to be extended by the user in order to
      implement this feature.
   */

extern int  expr_scan_GetLine ARGS ((int yyFile, char * yyBuffer, int yySize));

   /*
      expr_scan_GetLine is called from the scanner in order to fill a buffer at
...
# if HAVE_WCHAR_T
extern int  expr_scan_GetWLine ARGS ((int yyFile, wchar_t * yyBuffer, int yySize));

   /*
      Same as expr_scan_GetLine for type wchar_t instead of type char.
   */
# endif

extern void expr_scan_CloseSource ARGS ((int yyFile));

   /*
      expr_scan_CloseSource is called from the scanner function expr_scan_CloseFile
      at end of file or at end of input, respectively.
      It can be used to close files.
      The functions expr_scan_BeginSource... and expr_scan_CloseSource can be called
      ...
      Therefore after a call of expr_scan_CloseSource the properties of the
